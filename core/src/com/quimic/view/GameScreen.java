...
//*************************************************************//	
	...
	protected int QTD_INFO   = 4; // Valor default da quantidade de itens para informa��o/ajuda do jogo
...
//*************************************************************//		
	protected Stage          stage; // Controla e reage �s entradas do usu�rio	
	protected Stage          stageInfo; // Controla e reage �s entradas do usu�rio para a tela de informa��es
...
//*************************************************************//			
	...
	protected boolean menuFront; // Flag para sinalizar que o dialog de sair foi ativado e ficar� na frente de todos os componentes desenhados
...
//*************************************************************//		    
	...
	public GameScreen(QuimiCrush parent) {		
		...
		// Os elementos com duas combina��es
		...
		// Configura o componente que ira desenhar uma regi�o escura na tela enquanto n�o for o turno do jogador  
	...
	/**
	 * Recupera a sequ�ncia de sprites n�merados de '01' at� um determinado n�mero
	 * 
	 * @param name
	 * @param begin
	 * @param end
	...
	public void show() {
		...
		// Op��es da area de informa�oes -> menu lateral
	...
	protected void infoGamePanelAction() {			
		...
		// Evento para mostrar as combina��es poss�veis no jogo
		...
		// Evento para fechar a p�gina de informa��es
	...
	/**
	 * Cria as páginas com informações sobre as combinaçõs e sobre os compostos químicos
	 */
	...
	protected void drawHero() {		
		...
		if (heroAnimationFinish && battleState != GAME_END) { // hero.idle
			...
			heroAnimationFinish = false;
			if (! heroAttackAnimation.isAnimationFinished(stateTime)) { // Anima��o her�i ainda n�o terminou
				batch.draw((TextureRegion) heroAttackAnimation.getKeyFrame(stateTime, true), x, y);										
			} else { // Anima��o her�i terminou
			...
			if (! heroDamageAnimation.isAnimationFinished(stateTime)) { // Anima��o her�i ainda n�o terminou
				batch.draw((TextureRegion) heroDamageAnimation.getKeyFrame(stateTime, true), x, y);				
			} else { // Anima��o her�i terminou
				// Reduz uma vida do her�i 
				if (--lifeHero <= 0) {
			...
			if (! heroDieAnimation.isAnimationFinished(stateTime)) { // Anima��o her�i ainda n�o terminou
		...
		if (battleState != GAME_END) {
			// Desenhar a vida do her�i
			for (int heartFill = 0; heartFill < lifeHero; heartFill++) { // Desenha os cora��es cheios
				batch.draw(heart, x - heartFill*heart.getRegionWidth(), yHeart);
			}
			for (int heartNFill = LIFE_HERO; heartNFill > lifeHero && heartNFill > 0 ; heartNFill--) { // Desenha os cora��es vazios
	...
	protected void drawEnemy() {
		...
		if (enemyAnimationFinish && battleState != GAME_END) { // enemy.idle
			...
			if (! enemyAttackAnimation.isAnimationFinished(stateTime)) { // Anima��o inimigo ainda n�o terminou				
				batch.draw((TextureRegion) enemyAttackAnimation.getKeyFrame(stateTime, true), x-20f, y); // Avan�o do inimigo e atack								
			} else { 				
			...
			if (! enemyDamageAnimation.isAnimationFinished(stateTime)) { // Anima��o inimigo ainda n�o terminou
				batch.draw((TextureRegion) enemyDamageAnimation.getKeyFrame(stateTime, true), x, y);				
			} else { // Anima��o inimigo terminou
			...
			if (! enemyDieAnimation.isAnimationFinished(stateTime)) { // Anima��o inimigo ainda n�o terminou
		...
		if (battleState != GAME_END) {
			...
			for (int heartFill = 0; heartFill < lifeEnemy; heartFill++) { // Desenha os cora��es cheios
				batch.draw(heart, x + heartFill*heart.getRegionWidth(), yHeart);
			}
			for (int heartNFill = LIFE_ENEMY; heartNFill > lifeEnemy && heartNFill > 0 ; heartNFill--) { // Desenha os cora��es vazios
