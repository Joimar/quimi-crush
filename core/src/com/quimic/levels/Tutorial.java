...
//*************************************************************//
	...
	private AtlasRegion arrow; // M�o com o dedo apontando -> utilizado para o tutorial
	...
	private Stage noobStage; // Controla e reage �s entradas do usu�rio para a etapa de historia e tutorial	
...
//*************************************************************//	
	...
	public Tutorial(QuimiCrush parent) {				
	    ...
	    // Configura as vidas do heroi e vil�o, respectivamente
		LIFE_HERO = 2;
		LIFE_ENEMY = 1;
	    
	    noobStage = new Stage(stage.getViewport()); // Cria o palco para a etapa de hist�ria e tutorial
	    
	    // Configura a largura e altura do bloco quimico 
 		WIDTH_TILE  = (parent.windowWidth * PROPORTION_WIDTH_GAME) / sizeMapW; // Configura a largura do bloco que contem o componente qu�mico
 		HEIGHT_TILE = (parent.windowHeight * PROPORTION_HEIGHT_GAME) / sizeMapH; // Configura a altura do bloco que contem o componente qu�mico

 		// Configura a largura e altura dos itens na area de informa��es 
		...
		// Cria os objetos que cont�m as informa��es sobre os blocos do jogo, tanto os de ajuda quanto os para combina��o
		tiles = new Tile[sizeMapW][sizeMapH]; // Cria a matriz para as imagens dos blocos qu�micos quim�cos 
		/*tilesInfo = new Tile[QTD_INFO];
		for (int i = 0; i < QTD_INFO; i++) { // Insere as informa��es nos objetos dos itens de ajuda
			tilesInfo[i] = new Tile(new Sprite(elementsT.get(H2O+i)), H2O+i, WIDTH_OBJECT_INFO*0.80f, HEIGHT_OBJECT_INFO*0.80f);
		}*/					 
		
		// Permitindo a apari��o do tutorial e da historia no inicio da fase
		//historyScene  = true;
		historyScene  = false;
		tutorialScene = true;				
		// Sequ�ncia de apari��o das etapas do tutorial
		tutorialPart = 0;			
		dialogPart   = 0;
		...
		// Carrega anima��es
		this.loadAnimations(); // Carrega as anima��es do jogo		
		this.loadHistoryAnimations(); // Carrega anima��es da hist�ria
		this.loadTutorialAnimations(); // Carrega anima��es do tutorial						
	...
	/**
	 * Carrega as imagens e anima��es para a batalha no jogo	
	 */
	private void loadAnimations() {
		heart      = atlas.findRegion("heart");    // Captura a imagem de cora��o cheio
		nonHeart   = atlas.findRegion("heart-bg"); // Captura a imagem de cora��o vazio		
				
		Array<TextureRegion> regionsT;
		// Anima��es do hero		
		hero_idle = atlas.findRegion("hero_idle"); // Her�i parado 
		// ...
		regionsT = this.addRegionsArray("attack_", 1, 4);
		heroAttackAnimation = new Animation(0.2f, regionsT, PlayMode.NORMAL); // Atack do her�i
		// ..
		regionsT = this.addRegionsArray("damage1_", 1, 2);
		heroDamageAnimation = new Animation(0.3f, regionsT, PlayMode.NORMAL); // Dano no her�i
		// ...
		regionsT = this.addRegionsArray("die_", 1, 3);
		heroDieAnimation = new Animation(0.6f, regionsT, PlayMode.NORMAL); // Her�i morto
		hero_die = atlas.findRegion("hero_die");                           // Her�i caido
		// ...
		regionsT = this.addRegionsArray("hero_win_", 1, 2); 
		heroWinAnimation = new Animation(0.4f, regionsT, PlayMode.LOOP); // Pose da vit�ria
	...
	/**
	 * Carrega as imagens e anima��es da hist�ria do jogo
	 */
	...
	/**
	 * Carrega as imagens e anima��es do tutorial
	 */
	private void loadTutorialAnimations() {
		arrow = atlas.findRegion("dedoTutorial"); // Captura a imagem da m�o com o dedo
		timeArrow = 0;
	}

	/**
	 * Adiciona os eventos de avan�ar na historia/tutorial ('> proximo') e 	
	 * de pular historia ('>> pular')
	 */
	private void eventsListener() {
		// Adicionando evento para o "bot�o" de avan�ar na historia				
		...
		// Adicionando evento para o "bot�o" de pular historia
	...
	/**
	 * Respons�vel por atribuir os eventos para cada bloco na �rea do jogo (os blocos quimicos das combina��es)
	 */
		...
		for (int i = 0; i < game.getCells().size; i++) {
			game.getCells().get(i).getActor().addListener(new ClickListener() {
				...
				public void clicked(InputEvent event, float x, float y) {															
					...
					// Verificando por combina��es de elementos
					...
					if ( !(logic.combined || logic.matched) ) {
						if (!currentTile.destroy) { // if (currentTile.type != 100) {
							...
							if (logic.activeXY == 5 && tutorialPart == 4) {
								...
								wait = false;
								tutorialPart++; // 8
							} else if (logic.activeXY == 4 && (tutorialPart <= 7 || tutorialPart > 10)) { // N�o est� seguindo o tutorial
								logic.tileIsActive = false;
								previousTile.activated = false; // N�o permite sair da sequ�ncia do tutorial
								currentTile.activated = false; // N�o permite sair da sequ�ncia do tutorial
							} else if (logic.activeXY == 8) { // N�o est� seguindo o tutorial
								logic.tileIsActive = false;
								previousTile.activated = false; // N�o permite sair da sequ�ncia do tutorial
								currentTile.activated = false; // N�o permite sair da sequ�ncia do tutorial
							}
	...
	public void show() {
		...
		// Cria��o das tiles -> os componentes qu�micos -> Para a fa se tutorial predefinida
		Tile newTile = 				
		...
		//AQUI!!!
		// Hist�ria e tutorial
		beginTable = new Table(skin); // Cria��o da tabela para a hist�ria/tutorial 
		...
		historia = new Label("um ser roubou o cristal que dava vida \n " +
				...
				"daí ele foi atrás desse grupo. Pra isso ele \n " +
				"precisava dominar alguns elementos para conseguir \n" +
				" ir derrotando os vilões" , skin , "bold");

	/*	new Dialog("", skin) {
			{
				text("um ser roubou o cristal que dava vida ao vilarejo dele \n " +
						"e destruiu tudo que sobrou, daí ele foi atrás desse grupo.\n"+
						"Pra isso ele precisava dominar alguns elementos para conseguir ir derrotando os vilões");
			//	button("Ok", "");
				this.setMovable(false);
			}
		...
		tutorialWindow = new Window("", skin); // Cria��o de uma janela para inserir informa��es
	...
	public void render(float delta) {
		...
		if (! historyScene) { 
			...
			// Segue o fluxo da l�gica do jogo
			logic.update();
			...
			if (battleState == GAME_CONTINUE) {
				...
				if (battleState == GAME_WIN) {					
					// Evita que perca a �ltima fase concluida e extrapole o n�mero total de fases
	...
	private void drawDialogTutorial() {
		...
		wait = true;
		switch (dialogPart) {					
			case  0: // Mostrando a regi�o do jogo /game 				
				...
				break;
			case 1: // Mostrando a regi�o da batalha										
				...
				break;				
			case 2: // Mostrando a regi�o do catalogo de informa��es de combina��es						
			...
			case 3: // Informando sobre selecionar um componente
				...
				break;
			case 4: // Informando sobre selecionar outro componente para combina��o simples (combine)
				/*w = (parent.windowWidth*PROPORTION_WIDTH_GAME)*0.7f; 
				h = (parent.windowHeight*PROPORTION_HEIGHT_GAME)*0.7f;
				x = ((parent.windowWidth*PROPORTION_WIDTH_GAME) - w) / 2;
				y = ((parent.windowHeight*PROPORTION_HEIGHT_GAME) - h) / 2;*/
			...
			case 5: // Informando sobre selecionar o componente combinando anteriormente 
				...
				break;
			case 6: // Informando sobre selecionar o �ltimo componente para combina��o master (match)			
				new Dialog("", skin) {
					{
